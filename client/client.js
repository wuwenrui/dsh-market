window.__ModuleLoader__.load({ id: "@lawyer-dsh/market", factory: (require) => {
		...
		*/
		...
		//#endregion
		//#region src/client/billing-entry.ts
		/** Optional product-plugin UI contract. This event opens a panel only; it never submits an order. */
		const BILLING_OPEN_EVENT = "lawyer-billing:open";
		function openBillingPanel(target = window) {
		...
		var ManagedMarket_module_css_default = {
			...
			"cardSettings": "_1FIEtG_cardSettings",
			...
			"progressCard": "_1FIEtG_progressCard",
		...
		//#endregion
		//#region src/client/managed-locales.ts
		const managedZh = {
			market: "律师能力",
			models: "模型服务",
			loading: "正在加载…",
			...
			restart: "变更已通过启动检查。请关闭并重新启动工作台后使用新能力；重启前可以继续安装其他能力。",
			failed: "操作未完成",
			revision: "目录版本",
			...
			progressOf: "步骤",
			progressWait: "同一时间只处理一个安装操作；完成后可以继续安装其他能力，重启后一并生效。",
			restartTitle: "安装已完成，重启后生效",
			restartBody: "环境检查已通过。新能力需要重启工作台才会加载；重启前可以继续安装其他能力，重启后一并生效。",
			restartNow: "立即重启",
			restarting: "正在重启…",
		...
		const managedEn = {
			...
			restart: "The new environment passed its startup check. Close and restart the workbench to use the changes; you can keep installing other capabilities meanwhile.",
			...
			progressOf: "Step",
			progressWait: "One install runs at a time; when it finishes you can install another, and a single restart activates them all.",
			restartTitle: "Installed — restart to activate",
			restartBody: "The environment check passed. New capabilities load after a workbench restart; you can keep installing others meanwhile, and one restart activates them all.",
		...
		*/
		...
		function OperationProgress({ t, stage, variant = "panel" }) {
			const current = stageIndex(stage);
			const total = STAGES.length;
			const label = (index) => t(STAGE_LABEL[STAGES[index] ?? "preparing"]);
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				className: `${ManagedMarket_module_css_default.progress}${variant === "card" ? ` ${ManagedMarket_module_css_default.progressCard}` : ""}`,
				role: "status",
				"aria-live": "polite",
				children: [
		...
		*/
		...
		function ManagedMarketSection({ t, renderSlot }) {
			...
			*  「重启后生效」不是继续安装的前置条件，所以这个状态只表示「当前这一个操作」。 */
			const [running, setRunning] = (0, react.useState)(null);
			/** 本次页面生命周期内刚装/刚更新的能力：卡片上要留一条「已更新，需要重启」。
			*  重启会重新加载页面，这个集合自然清空，不需要持久化。 */
			const [updatedIds, setUpdatedIds] = (0, react.useState)([]);
			...
			*  卸载仍然要确认，因为它是拿掉既有能力。 */
			...
			const run = async () => {
				...
				setPending(null);
				if (await start(operation)) await load();
			};
			...
			*  进度画在正在处理的那张卡片上，已完成的卡片留「已更新，需要重启」。 */
			const runAll = async () => {
				...
				const targets = installed.flatMap((item) => {
					...
					return known === void 0 || known.version === item.version || !known.compatible ? [] : [{
						item,
						known
					}];
				});
				...
				for (const { item, known } of targets) {
					...
					setBatch((previous) => previous === null ? null : {
						...previous,
			...
			*  没贡献面板的能力展开后给一句说明，而不是给一个空白框。 */
			...
			const progressFor = (id) => running !== null && running.id === id ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(OperationProgress, {
				t,
				stage: status.stage,
				variant: "card"
			}) : null;
				...
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("header", {
						...
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("h2", {
							...
							children: [updatableCount > 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
								type: "button",
								className: ManagedMarket_module_css_default.actionPrimary,
								onClick: () => void runAll(),
								disabled: busy,
								children: batch === null ? `${t("updateAll")} (${updatableCount})` : `${t("updatingAll")} ${batch.done}/${batch.total}`
					...
					running !== null && !runningVisible ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(OperationProgress, {
						t,
						stage: status.stage
					}) : null,
						...
						children: installed.length === 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							...
							children: installedRows.map((item) => {
								...
								return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(CapabilityCard, {
									...
									description: known?.description ?? "",
									badges: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
										/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
											className: ManagedMarket_module_css_default.badge,
											"data-kind": "installed",
										...
										updatedIds.includes(item.id) ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
											className: ManagedMarket_module_css_default.badge,
											"data-kind": "updated",
											children: t("updatedRestart")
										}) : null
									] }),
									...
									action: progressFor(item.id) === null ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
										renderSlot !== void 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
											...
											disabled: busy,
											children: expandedId === item.id ? t("hideSettings") : t("settings")
										}) : null,
										/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
											type: "button",
											className: ManagedMarket_module_css_default.actionGhost,
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)(CardColumns, {
								...
								children: availableRows.map((item) => {
									...
									return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(CapabilityCard, {
										...
										progress: progressFor(item.id),
										action: progressFor(item.id) !== null ? null : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
											type: "button",
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("p", {
						...
						children: [
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
								...
								onClick: () => {
									setError("");
									if (!openBillingPanel()) setError(t("billingMissing"));
								},
								children: t("account")
							})
						]
